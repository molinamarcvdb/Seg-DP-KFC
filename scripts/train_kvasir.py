#!/usr/bin/env python3
"""
Training script for Kvasir-SEG polyp segmentation experiments.

Supports:
- Baseline (no DP)
- DP-SGD without preconditioning
- DP-SGD with synthetic preconditioning (AdaDPS)
- DP-SGD with momentum preconditioning
- DP-SGD with public data preconditioning (oracle)

Usage:
    python scripts/train_kvasir.py --method baseline --epochs 20
    python scripts/train_kvasir.py --method dp --epsilon 8.0 --epochs 20
    python scripts/train_kvasir.py --method dp_synthetic --epsilon 8.0 --epochs 20
"""
...
from src.data import KvasirSegDataset
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train on Kvasir-SEG")

    # Method
    parser.add_argument("--method", type=str, default="baseline",
                        choices=["baseline", "dp", "dp_synthetic", "dp_momentum", "dp_public",
                                 "dp_kfac", "dp_kfac_synth",
    ...
    parser.add_argument("--mask_strategy", type=str, default=None,
                        choices=["gaussian_blobs", "random_shapes", "pink_threshold",
                                 "voronoi", "frangi"],
                        help="Mask strategy (overrides --mask_type if set)")
...
def main():
    ...
    exp_name = f"kvasir_{args.method}"
    ...
    # Data
    print("\nLoading Kvasir-SEG...")
    train_dataset = KvasirSegDataset(split="train", image_size=args.image_size, augment=True, seed=args.seed)
    val_dataset = KvasirSegDataset(split="val", image_size=args.image_size, augment=False, seed=args.seed)
    ...
    if args.method == "baseline":
    ...
    elif args.method in ["dp_synthetic", "dp_momentum"]:
        print(f"  epsilon={args.epsilon}, preconditioning from synthetic data")

        if args.method == "dp_synthetic":
            precond = AdaDPSPreconditioner(damping=args.damping, device=args.device)
        else:
            precond = MomentumPreconditioner(damping=args.damping, beta2=args.beta2,
                                             device=args.device)
    ...
    elif args.method in ["dp_kfac", "dp_kfac_synth"]:
        is_public = args.method == "dp_kfac"
        source = "train data (oracle)" if is_public else "synthetic data"
        print(f"  epsilon={args.epsilon}, K-FAC preconditioning from {source}")

        precond = KFACPreconditioner(damping=args.kfac_damping, device=args.device)
        wrapped = GradSampleModule(model).to(args.device)
        ...
        if is_public:
            ...
            precond.estimate_from_synthetic(wrapped, loss_fn, num_samples=args.precond_samples,
                                           batch_size=args.batch_size, image_size=args.image_size,
                                           in_channels=3, num_classes=1,
                                           noise_type=args.noise_type, task="segmentation",
